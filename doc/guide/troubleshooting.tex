...
\begin{enumerate}
\item
  Check orientation of the \url{SAM4S}.
\item
  Check soldering of the \url{SAM4S} pins under a microscope. Giving
...
\item
  ...
  booting from flash, and reprogram the \url{SAM4S} with an LED flash
  program.
\end{enumerate}
...
\url{OpenOCD} periodically polls to see if the \url{SAM4S} is alive.
...
\url{OpenOCD} will then fail to communicate with the SAM4S.
...
\item
  Check the definition in the configuration file \texttt{target.h}
\end{enumerate}
...
\item
  ...
  data (TDA) signals.
\item
  TWI channel 1 (TWI1) uses PB4 and PB5. However, these are configured
...
\begin{itemize}
\item
  \emph{\url{OpenOCD} does not run}. The most common problem is that the
...
\item
  ...
  \item
    ...
    pointers (say by not calling \texttt{radio.begin}). Try running
